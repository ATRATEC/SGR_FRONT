 ...
 */
  ...
  ngOnInit() {
    ...
    Observable.combineLatest(idFilter$, descricaoFilter$, clienteFilter$,
                            vigenciaInicioFilter$, vigenciaFinalFilter$).debounceTime(500).distinctUntilChanged().
    map(
      ([id, descricao, cliente, vigencia_inicio, vigencia_final]) =>
      ({id, descricao, cliente, vigencia_inicio, vigencia_final})).subscribe(filter => {
