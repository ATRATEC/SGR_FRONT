...
import { DsFornecedorDocumento } from './dsfornecedordocumento';
...
@Component({
  selector: 'app-fornecedordocumento',
  templateUrl: './fornecedordocumento-list.component.html',
  styleUrls: ['./fornecedordocumento-list.component.css']
})
export class FornecedorDocumentoListComponent implements OnInit, AfterViewInit {
  displayedColumns = ['id', 'descricao', 'razao_social', 'numero', 'emissao', 'vencimento'];
  // displayedColumns = ['id', 'codigo', 'descricao'];
  dataSource: DsFornecedorDocumento | null;
  selectedRowIndex = -1;
  ...
  obterFornecedorDocumentos() {
    // const token = this._tokenManager.retrieve();
    // this._fornecedordocumentoService.getFornecedorDocumentos(token).subscribe(data => {
    //   this.fornecedordocumentos = data.data;
    //   console.log(data);
    //   console.log(this.fornecedordocumentos.length);
    //   console.log(token);
    // });
  }
  ...
  dblck(row) {
    this._router.navigate(['/fornecedordocumentos/fornecedordocumento', {id: row.id}]);
  }
 ...
 */
  ...
  incluirRegistro() {
    this._router.navigate(['/fornecedordocumentos/fornecedordocumento']);
  }


  editarRegistro() {
    if (this.validaSelecao()) {
      this._router.navigate(['/fornecedordocumentos/fornecedordocumento', {id: this.selectedRow.id}]);
      this.ngOnInit();
      this.selectedRowIndex = -1;
      this.selectedRow = null;
    }
        ...
        result => {
          if (result.retorno) {
            this._fornecedordocumentoService.deleteFornecedorDocumento(this._tokenManager.retrieve(), this.selectedRow.id).subscribe(
              data => {
                this.dialog.success('SGR', 'FornecedorDocumento excluído com sucesso.');
                this.ngOnInit();
              },
              error => {
  ...
  ngOnInit() {
    ...
    this.dataSource = new DsFornecedorDocumento(this._tokenManager, this._fornecedordocumentoService, this.paginator, this.sort);
